...
Emergency Stop can terminate a process during an atomic action. The action may have partially changed external state before termination. Fast Hands therefore records `needsReview: true` and requires explicit acknowledgement before retrying/resuming that uncertain step.
